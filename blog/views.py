...
def attedance_detail(request, team_id):
...
def attedance_take(request, team_id):
    today = date.today()
    team = Team.objects.get(id=team_id)
...
def attendance_update(request, attendance_id):
    ...
    if attendance.date == date.today():
        ...
        return HttpResponse("Faqatkina bugungi davomat uchun tahrirlash mumkin!")